...
# runtime flags
INTERFACES := eth0
MOUNTFLAGS := # -v $(CURDIR)/config:/etc/dhcp
PORTFLAGS  := -p 67:67/udp -p 67:67/tcp# --net=host # so other local containers can find it without explicit linking, needs firewall cleared
PUID       := $(shell id -u)
...
$(shell docker run --rm --pull=never \
	...
	--version \
	2>&1 \
	| grep 'isc-dhcpd-' | sed -e 's_isc-dhcpd-__')
endef
# -- }}}
